...
class ProxyService(proxyConfig: ProxyConfig,
  ...
  private def handleWebSocketRequest(targetHost: Host, request: HttpRequest, upgrade: UpgradeToWebSocket): Future[HttpResponse] = {
    ...
    val flow = Flow.fromSinkAndSourceMat(Sink.asPublisher[Message](fanout = false), Source.asSubscriber[Message])(Keep.both) map { msg =>
      logger.debug(s"Got websocket message $msg")
      msg
    }
